...
static void bone_dump_context_for_vb(int frame) {
    ...
    for (int i = start; i < g_bone_log_head; i++) {
        ...
        char b[256];
        wsprintfA(b,
            "[OUTLIER]  bone F%05d ret=0x%08X ptr=%p absmax=%d degen=%d eax=%u reason=%s",
            e->frame, e->caller_ret, e->actual_ptr,
            (int)e->out_absmax, e->degenerate, e->fn_eax, e->reason);
        log_f(b);
    }
}
...
static HRESULT WINAPI hook_VBUnlock(void* self) {
    ...
    if (e && e->real_ptr && e->lock_sz > 0 && e->stride >= 4) {
        ...
        if (e->lock_fvf & D3DFVF_XYZRHW) {
            ...
            /*
             * --- Phase 2: Dragon-particle suppression + NEAR_OUTLIER diagnostic ---
             *
             ...
             *   a) quad_max_xy > VB_OUTLIER_THRESH -> SUPPRESS (collapse+alpha=0)
             *   b) bbox extends beyond near-outlier bounds (but below threshold)
             *      -> [NEAR_OUTLIER] logged, NOT suppressed.
             *      Determines whether box-bleed comes from sub-5000 particles
             *      or from Matrix-B / non-XYZRHW draw path.
             *
             * Near-outlier bounds:
                ...
                for (UINT ci = 0; ci < chunks; ci++) {
                    ...
                    if (quad_max_xy > VB_OUTLIER_THRESH) {
                        ...
                        verts_suppressed += 4;
                               ...
                               w > 1500.0f       || h > 1500.0f) {
                        /* Near-outlier suppress: quad confirmed off-screen by
                         * [NEAR_OUTLIER] diagnostic -- now promoted to suppress.
                         * Log as SUPPRESS-NEAR to distinguish from hard path. */
                        if (quad_max_xy > suppress_max_xy)
                            suppress_max_xy = quad_max_xy;
                        for (UINT j = 0; j < 4; j++) {
                            UINT   vi  = base + j;
        ...
        if (params && params->BackBufferWidth && params->BackBufferHeight) {
             ...
             */
            {
                ...
                if (vp >= 14.0f/9.0f - 0.01f && vp <= 14.0f/9.0f + 0.01f) {
                    ...
                    sc_sin = 0.401797f; sc_cos = 0.915731f;  /* 16:9 */
...
BOOL WINAPI DllMain(HINSTANCE h, DWORD reason, LPVOID r) {
    ...
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        log_line("d3d8 widescreen proxy v16-lite-r7 loaded");
